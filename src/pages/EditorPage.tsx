...
export default function EditorPage() {
  ...
  const [label, setLabel] = useServerState<LabelData>(
    '/api/me/label',
    '/api/me/label',
    createBlankLabel(),
    (r) => r?.label ?? null,
  );
  const [templates, setTemplates] = useServerState<Template[]>(
    '/api/me/templates',
    null,
    PRESET_TEMPLATES,
    (r) => (Array.isArray(r?.templates) ? r.templates : null),
  );
  const [customIngredients, setCustomIngredients] = useServerState<Ingredient[]>(
    '/api/me/custom-ingredients',
    '/api/me/custom-ingredients',
    [],
    (r) => (Array.isArray(r?.ingredients) ? r.ingredients : null),
  );
  ...
  useEffect(() => {
    setLabel((l) => {
      ...
      const next = { ...l };
      let changed = false;
      if (typeof next.productDescription !== 'string') {
        next.productDescription = '';
        changed = true;
      }
      if (typeof next.bakedDate !== 'string') {
        next.bakedDate = '';
        changed = true;
      }
      if (!next.typography) {
        next.typography = blank.typography;
        changed = true;
      }
      if (!next.sections) {
        next.sections = blank.sections;
        changed = true;
      } else {
        const sd = blank.sections;
        const merged = { ...sd, ...next.sections };
        if (Object.keys(merged).length !== Object.keys(next.sections).length) {
          next.sections = merged;
          changed = true;
        }
      }
      if (!next.fields) {
        next.fields = defaultFieldStyles();
        changed = true;
      } else {
        // Ensure every key exists (forward-compat).
        const defaults = defaultFieldStyles();
        const merged = { ...defaults, ...next.fields };
        if (Object.keys(merged).length !== Object.keys(next.fields).length) {
          next.fields = merged;
          changed = true;
        }
      }
  ...
  const handlePdf = async () => {
    try {
      await exportLabelsToPdf(label, label.copies);
    } catch (e) {
      console.error(e);
  ...
  const handlePng = async () => {
    try {
      await exportLabelToPng(label, true);
    } catch (e) {
      console.error(e);
      alert('Kunde inte exportera PNG.');
    }
  };

  const handleReset = () => {
    if (confirm('Återställ etikett till tom mall?')) {
      setLabel(createBlankLabel());
    }
  };
  ...
  const saveTemplate = async (name: string) => {
    ...
    try {
      await api.post('/api/me/templates', { id: tpl.id, name: tpl.name, data: tpl.data });
    } catch (e) { console.error(e); }
  };

  const deleteTemplate = async (id: string) => {
    if (!confirm('Radera mall?')) return;
    setTemplates(templates.filter((t) => t.id !== id));
    try { await api.del(`/api/me/templates/${id}`); } catch (e) { console.error(e); }
  };

  const duplicateTemplate = async (id: string) => {
    const t = templates.find((x) => x.id === id);
    if (!t) return;
    const copy: Template = { ...t, id: uuid(), name: `${t.name} (kopia)`, createdAt: Date.now() };
    setTemplates([copy, ...templates]);
    try {
      await api.post('/api/me/templates', { id: copy.id, name: copy.name, data: copy.data });
    } catch (e) { console.error(e); }
  };
        ...
        <section className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Produktbeskrivning" className="col-span-2" hint="Kort beskrivning som visas under produktnamnet. Stöder radbrytning.">
                <textarea
                  className="input min-h-[44px] resize-y"
                  rows={2}
                  value={label.productDescription ?? ''}
              ...
              <Field label="Bäst före">
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    onClick={() => updateLabel('bestBefore', todayIso())}
                  >
                    Idag
                  </button>
                </div>
              ...
              <Field label="Bakat">
                <div className="flex gap-2">
                  ...
                  <button
                    type="button"
                    className="btn whitespace-nowrap text-xs"
                    onClick={() => updateLabel('bakedDate', todayIso())}
                  >
                    Idag
                  </button>
                </div>
          ...
          <div className="card p-4">
            ...
            <div className="flex flex-col gap-3">
              <IngredientPicker
                ...
                onCreateCustom={(ing) =>
                  setCustomIngredients([{ ...ing, id: `custom-${ing.name}` }, ...customIngredients])
                }
              />
              <IngredientList
                items={label.ingredients}
                onChange={(items) => updateLabel('ingredients', items)}
              />
              <div>
                <span className="label">Upptäckta allergener</span>
                <div className="mt-1.5">
                  <AllergenLegend active={allergens} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="card p-4">
              ...
              <LabelSizeSelector
                value={label.size}
                onChange={(s) => updateLabel('size', s)}
                onCustom={(s) => updateLabel('size', s)}
                layout={label.layout}
          ...
          <div className="card p-4">
            ...
            <TypographyPanel
              ...
              onTypography={(t) => updateLabel('typography', t)}
              sections={label.sections}
              onSections={(s) => updateLabel('sections', s)}
            />
          </div>
        ...
        <section className="flex flex-col gap-4 lg:sticky lg:top-[72px] lg:self-start">
          <div className="card p-4">
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-ink/60">
              <span>
                Storlek: <strong className="text-ink/80">{label.size.widthMm} × {label.size.heightMm} mm</strong>
              </span>
              <span>
                Layout: <strong className="text-ink/80 capitalize">{label.layout}</strong>
              </span>
              <span>
                Senast ändrad: {new Date(label.updatedAt).toLocaleTimeString('sv-SE')}
              </span>
            </div>
          </div>
